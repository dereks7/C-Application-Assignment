...
struct Token 
{
  ...
  Token(char ch, string val) :kind(ch),name(val) { } // Error 1. name was missing
};
...
Token Token_stream::get()
{
		...
		if (isalpha(ch) || ch == '_')  // allow _'s in names
    {
			...
			if (s == "quit") return Token(quit); // error was return Token(name)
			return Token(name, s);
		}
		error("Bad token");
...
double primary()
{
	...
	{	double d = expression();
	t = ts.get();
	if (t.kind != ')') error("')' expected"); // error: had error "(" instead of ")"
  return d; // was not returning anything
	}
	case '-':
		return -primary();
	case number:
